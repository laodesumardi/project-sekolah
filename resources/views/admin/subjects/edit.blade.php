...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-600">Edit informasi mata pelajaran</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form method="POST" action="{{ route('admin.subjects.update', $subject) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="grade_level" 
                        ...
                        <option value="10" {{ old('grade_level', $subject->grade_level) === '10' ? 'selected' : '' }}>Kelas 10</option>
                        <option value="11" {{ old('grade_level', $subject->grade_level) === '11' ? 'selected' : '' }}>Kelas 11</option>
                        <option value="12" {{ old('grade_level', $subject->grade_level) === '12' ? 'selected' : '' }}>Kelas 12</option>
                        <option value="all" {{ old('grade_level', $subject->grade_level) === 'all' ? 'selected' : '' }}>Semua Tingkat</option>
                    </select>
                    @error('grade_level')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" 
                              ...
                              placeholder="Masukkan deskripsi mata pelajaran">{{ old('description', $subject->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center p-4 bg-gray-50 rounded-lg">
                            ...
                            <a href="{{ $subject->syllabus_url }}" 
                               target="_blank"
                               class="ml-4 inline-flex items-center px-3 py-2 bg-primary-500 text-white text-sm rounded-lg hover:bg-primary-600 transition-colors duration-200">
                ...
                <div class="md:col-span-2">
                    <label for="syllabus_file" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $subject->syllabus_file ? 'Ganti Silabus (PDF)' : 'Upload Silabus (PDF)' }}
                    </label>
