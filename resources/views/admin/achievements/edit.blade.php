...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-600">Edit informasi prestasi</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form method="POST" action="{{ route('admin.achievements.update', $achievement) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="category" 
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category }}" {{ old('category', $achievement->category) === $category ? 'selected' : '' }}>{{ $category }}</option>
                        @endforeach
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="rank" 
                        ...
                        @foreach($ranks as $rank)
                            <option value="{{ $rank }}" {{ old('rank', $achievement->rank) === $rank ? 'selected' : '' }}>{{ $rank }}</option>
                        @endforeach
                    </select>
                    @error('rank')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="participant_type" 
                        ...
                        <option value="individual" {{ old('participant_type', $achievement->participant_type) === 'individual' ? 'selected' : '' }}>Individu</option>
                        <option value="team" {{ old('participant_type', $achievement->participant_type) === 'team' ? 'selected' : '' }}>Tim</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('participant_names', $achievement->participant_names) }}"
                ...
                <div>
                    ...
                    <input type="date" 
                           id="date" 
                           name="date" 
                           value="{{ old('date', $achievement->date->format('Y-m-d')) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" 
                              ...
                              placeholder="Masukkan deskripsi prestasi">{{ old('description', $achievement->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @if($achievement->certificate_image)
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center p-4 bg-gray-50 rounded-lg">
                            <img src="{{ $achievement->certificate_url }}" alt="{{ $achievement->title }}" class="w-20 h-20 object-cover rounded-lg mr-4">
                            <div class="flex-1">
                                <p class="text-sm font-medium text-gray-900">{{ $achievement->title }} - Sertifikat</p>
                ...
                <div class="md:col-span-2">
                    <label for="certificate_image" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $achievement->certificate_image ? 'Ganti Sertifikat/Foto' : 'Upload Sertifikat/Foto' }}
                    </label>
