...
@section('page-title', 'Edit Event Kalender')
...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Event Kalender</h1>
            <p class="text-gray-600">Edit informasi event kalender akademik</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form method="POST" action="{{ route('admin.calendar.update', $calendar) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="event_type" 
                        ...
                        @foreach($eventTypes as $type)
                            <option value="{{ $type }}" {{ old('event_type', $calendar->event_type) === $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                        @endforeach
                    </select>
                    @error('event_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="academic_year_id" 
                        ...
                        @foreach($academicYears as $year)
                            <option value="{{ $year->id }}" {{ old('academic_year_id', $calendar->academic_year_id) == $year->id ? 'selected' : '' }}>{{ $year->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" 
                           id="end_date" 
                           name="end_date" 
                           value="{{ old('end_date', $calendar->end_date ? $calendar->end_date->format('Y-m-d') : '') }}"
                ...
                <div>
                    ...
                    <input type="time" 
                           id="start_time" 
                           name="start_time" 
                           value="{{ old('start_time', $calendar->start_time ? $calendar->start_time->format('H:i') : '') }}"
                ...
                <div>
                    ...
                    <input type="time" 
                           id="end_time" 
                           name="end_time" 
                           value="{{ old('end_time', $calendar->end_time ? $calendar->end_time->format('H:i') : '') }}"
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center">
                        <input type="checkbox" 
                               ...
                               name="is_all_day" 
                               value="1" 
                               {{ old('is_all_day', $calendar->is_all_day) ? 'checked' : '' }}
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" 
                           id="location" 
                           name="location" 
                           value="{{ old('location', $calendar->location) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" 
                              ...
                              placeholder="Masukkan deskripsi event">{{ old('description', $calendar->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
