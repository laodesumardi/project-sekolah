        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-lg p-4 lg:p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-4 rounded-lg {{ $setting->getRegistrationStatus() === 'open' ? 'bg-green-50 border border-green-200' : ($setting->getRegistrationStatus() === 'not_started' ? 'bg-yellow-50 border border-yellow-200' : 'bg-red-50 border border-red-200') }}">
                        <div class="flex items-center">
                            <div class="w-3 h-3 rounded-full {{ $setting->getRegistrationStatus() === 'open' ? 'bg-green-500' : ($setting->getRegistrationStatus() === 'not_started' ? 'bg-yellow-500' : 'bg-red-500') }} mr-3"></div>
                            <div>
                                <p class="font-semibold text-gray-900">
                                    @switch($setting->getRegistrationStatus())
                                        @case('open') Pendaftaran Sedang Berlangsung @break
                                        @case('not_started') Pendaftaran Belum Dimulai @break
                                        @case('ended') Pendaftaran Sudah Berakhir @break
                                        @case('inactive') Pendaftaran Tidak Aktif @break
                                        @default Status Tidak Diketahui @break
                                    @endswitch
                                </p>
                                <p class="text-sm text-gray-600">
                                    @if($setting->getRegistrationStatus() === 'not_started')
                                        Dimulai dalam {{ $setting->getDaysUntilStart() }} hari
                                    @elseif($setting->getRegistrationStatus() === 'open')
                                        Berakhir dalam {{ $setting->getDaysUntilEnd() }} hari
                                    @elseif($setting->getRegistrationStatus() === 'ended')
                                        Sudah berakhir {{ $setting->end_date->diffInDays(now()) }} hari yang lalu
                                    @endif
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <div>
                                <p class="font-semibold text-gray-900">Periode Pendaftaran</p>
                                <p class="text-sm text-gray-600">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <div>
                            ...
                            <input type="date" name="start_date" 
                                   ...
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <p class="text-xs text-gray-500 mt-1">Tanggal mulai tidak boleh di masa lalu</p>
                        </div>
                        <div>
                            ...
                            <input type="date" name="end_date" 
                                   ...
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <p class="text-xs text-gray-500 mt-1">Tanggal selesai harus setelah tanggal mulai</p>
                        </div>
                    </div>
