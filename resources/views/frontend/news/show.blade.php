...
<div class="relative">
    ...
    <div class="absolute bottom-0 left-0 right-0 p-6 text-white">
        <div class="max-w-4xl mx-auto">
            <h1 class="text-2xl md:text-4xl font-bold mb-4">{{ $news->title }}</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex space-x-2">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('news.show', $news)) }}" 
                               target="_blank"
                               class="inline-flex items-center justify-center w-8 h-8 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-colors duration-200">
                            ...
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('news.show', $news)) }}&text={{ urlencode($news->title) }}" 
                               target="_blank"
                               class="inline-flex items-center justify-center w-8 h-8 bg-blue-400 text-white rounded-full hover:bg-blue-500 transition-colors duration-200">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <button onclick="copyToClipboard('{{ route('news.show', $news) }}')"
                                    class="inline-flex items-center justify-center w-8 h-8 bg-gray-600 text-white rounded-full hover:bg-gray-700 transition-colors duration-200">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($news->tags as $tag)
                                <a href="{{ route('news.tag', $tag) }}" 
                                   class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium hover:opacity-80 transition-opacity duration-200" 
                                ...
                                <div class="flex space-x-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <a href="{{ route('news.show', $related) }}" 
                                           class="text-lg font-semibold text-gray-900 hover:text-primary-600 line-clamp-2">
                                            {{ $related->title }}
                                        </a>
            ...
            <div class="lg:col-span-1">
                <div class="bg-gray-50 rounded-lg p-6 sticky top-8">
                    <!-- Popular Tags -->
                    <div class="mb-8">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">Tag Populer</h3>
                        <div class="flex flex-wrap gap-2">
                            @foreach($popularTags as $tag)
                                <a href="{{ route('news.tag', $tag) }}" 
                                   class="inline-flex items-center px-2 py-1 rounded text-xs font-medium hover:opacity-80 transition-opacity duration-200" 
                                   style="background-color: {{ $tag->color }}20; color: {{ $tag->color }}">
                                    #{{ $tag->name }}
                                </a>
                            @endforeach
