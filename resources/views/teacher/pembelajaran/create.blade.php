...
@section('title', 'Buat Konten Pembelajaran')
                ...
                <ol class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-4 text-sm font-medium text-gray-500">Buat Konten Baru</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h2 class="mt-2 text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight">
                Buat Konten Pembelajaran
            </h2>
            <p class="mt-1 text-sm text-gray-500">
                Buat konten pembelajaran baru untuk siswa
            </p>
        </div>
    </div>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="interactive">Pembelajaran Interaktif</option>
                            <option value="experiment">Eksperimen Virtual</option>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4" required
                                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                                  placeholder="Masukkan deskripsi pembelajaran"></textarea>
                    </div>
                    <div>
...
async function submitForm(event) {
    ...
    try {
        const response = await fetch('{{ route("teacher.pembelajaran.store") }}', {
            method: 'POST',
            headers: {
        ...
        if (data.success) {
            alert(data.message);
            window.location.href = '{{ route("teacher.pembelajaran.index") }}';
        } else {
            alert(data.message || 'Gagal membuat konten pembelajaran.');
        }
    } catch (error) {
        console.error('Error:', error);
        alert('Gagal membuat konten pembelajaran. Silakan coba lagi.');
    } finally {
